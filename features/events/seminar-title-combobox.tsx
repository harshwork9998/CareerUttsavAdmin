...
type SeminarTitleComboboxProps = {
  value: string;
  onChange: (value: string) => void;
  id?: string;
};

export function SeminarTitleCombobox({
  value,
  onChange,
  id,
}: SeminarTitleComboboxProps) {
  ...
  const listboxId = id ? `${id}-listbox` : "seminar-title-listbox";
      ...
      <PopoverAnchor asChild>
        <div className="relative">
          <Input
            ...
            autoComplete="off"
            placeholder="Type to search seminars…"
            value={display}
            onChange={(e) => {
        ...
        >
          {filtered.length === 0 ? (
            <p className="px-3 py-2 text-sm text-muted-foreground">
              No matching seminars
            </p>
          ) : (
            filtered.map((seminar, index) => {
              const selected = seminar === value;
